...
/* ─────────────────────────────────────────────
   BACKGROUND ORBS (desktop/tablet only)
   These are decorative, infinitely-animating, heavily
   blurred blobs. Running them behind backdrop-filter
   cards on mobile forces the browser to re-sample the
   blur on every frame — the single biggest cause of
   mobile jank here — so they're disabled below md.
   A static, non-animated, much lighter glow is used
   for mobile instead (see MobileGlow).
───────────────────────────────────────────── */
function BackgroundOrbs() {
  return (
...
// Static, non-animated, low-cost glow for mobile — no infinite RAF loop,
// no large blur radius, nothing for backdrop-filter to keep re-sampling.
function MobileGlow() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden md:hidden">
      <div className="absolute left-1/2 top-[10%] h-56 w-56 -translate-x-1/2 rounded-full bg-[radial-gradient(circle,rgba(139,92,246,0.20)_0%,transparent_70%)] blur-2xl" />
      <div className="absolute -right-10 bottom-[15%] h-48 w-48 rounded-full bg-[radial-gradient(circle,rgba(168,85,247,0.16)_0%,transparent_70%)] blur-2xl" />
    </div>
  );
}
...
/* ─────────────────────────────────────────────
   MOBILE TIMELINE CARD — compact, sits beside an
   inline node dot, fades in from the left.

   Perf notes vs. the previous version:
   - No backdrop-filter: it's one of the most expensive
     paint operations on mobile GPUs, especially while
     the card itself is translating. Swapped for a solid
     tinted background that looks visually equivalent
     against the dark section background.
   - Connector line now animates `scaleY` (compositor-only)
     instead of `height` (triggers layout on every frame).
   - Card slide-in duration fixed from 2s -> 0.55s, and
     uses whileInView (Framer's built-in, more efficient
     viewport observer) instead of manual state + animate.
   - will-change + translateZ(0) hint GPU layer promotion
    ...
    <div className={`relative flex gap-3 ${isLast ? "" : "pb-5"}`}>
      ...
      <motion.div
        ...
        style={{
          ...
          borderColor: "rgba(255,255,255,0.12)",
          boxShadow:
            "0 6px 20px -6px rgba(0,0,0,0.4), inset 0 1px 1px rgba(255,255,255,0.06)",
